...
# Assert that these directories exist
if [ ! -d "$MAGMA" ]; then
    echo "ERROR: MAGMA directory does not exist: $MAGMA"
    exit 1
fi

if [ ! -d "$FUZZER" ]; then
    echo "ERROR: FUZZER directory does not exist: $FUZZER"
    exit 1
fi

AFL_SRC="$MAGMA/../AFLplusplus"
if [ ! -d "$AFL_SRC" ]; then
    echo "ERROR: AFLplusplus directory does not exist: $AFL_SRC"
    exit 1
fi

mkdir -p "$FUZZER/repo"
# copy contents, may want to use -rT "$AFL_SRC" "$FUZZER/repo" instead
cp -r "$AFL_SRC"/. "$FUZZER/repo"
